...
public class Step4 {
    ...
    public static void main( String args[] ) {
        ...
        List<BufferedImage> images = RectangleUtils.splitImages(image, cells);
        
        System.out.println("Total de Imagens: " + images.size());
        ListDisplayPanel panel = new ListDisplayPanel();
        
        int count = 0;
        for (BufferedImage cellimage : images) {
            panel.addImage(cellimage, count + " : " + cells.get(count).x0 + "x" + cells.get(count).y0);
            count++;
        }
